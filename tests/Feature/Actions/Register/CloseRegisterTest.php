...
it('opens register with zero balance', function () {
    ...
    $action->handle($this->register->code, 20.0);
...
it('throws exception and updates balance with credit history when opening register with non-zero balance', function () {
    ...
    expect(fn () => $action->handle($this->register->code, $value))->toThrow(RegisterAttemptException::class);

    $action->handle($this->register->code, $value);
